...
plot_learning=function(pA,learning_rate,temperature){
  ...
  for (i in 1:nblocks) {
    ...
    block_optimal[i]=mean(model$resp[block_trials]==round(pA))
  }
  plot(block_optimal,type='l',xlab=sprintf('trial blocks (%d trials/block)',blocksize),
       ylab='proportion optimal responses')
...
renderPlot({
  plot_learning(input$pA,input$learning_rate,input$temperature)
  })
```
